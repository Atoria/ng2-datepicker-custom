...
export class DatePickerComponent implements ControlValueAccessor, OnInit {
  ...
  generateYears() {
    const date = (this.minDate ? this.minDate.clone() : null) || Moment().year(Moment().year() - 40);
    const toDate = (this.maxDate ? this.maxDate.clone() : null) || Moment().year(Moment().year() + 40);
